...
func httptestServer(t *testing.T) *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var testReq gokitjsonrpctransport.Request
...
func TestClientHappyPath(t *testing.T) {
	...
	var (
		...
		decode = func(ctx context.Context, res gokitjsonrpctransport.Response) (interface{}, error) {
	...
	var requestAtServer gokitjsonrpctransport.Request
	...
	if requestAtServer.JSONRPC != gokitjsonrpctransport.Version {
		t.Fatalf("JSON-RPC version at server: want=%s, got=%s", gokitjsonrpctransport.Version, requestAtServer.JSONRPC)
	}
...
func TestClientCanHandleJSONRPCError(t *testing.T) {
	...
	sut := jsonrpc.NewClient[any, any](mustParse(server.URL), "add")
...
func TestDefaultAutoIncrementer(t *testing.T) {
	...
	sut := gokitjsonrpctransport.NewAutoIncrementID(0)
	var want uint64
	for ; want < 100; want++ {
		got := sut.Generate()
		if got != want {
