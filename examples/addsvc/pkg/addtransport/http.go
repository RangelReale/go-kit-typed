...
import (
	...
	"github.com/RangelReale/go-kit-typed/examples/addsvc/pkg/addendpoint"
	"github.com/RangelReale/go-kit-typed/examples/addsvc/pkg/addservice"
)
...
func NewHTTPHandler(endpoints addendpoint.Set, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, logger log.Logger) http.Handler {
	...
	m := http.NewServeMux()
	m.Handle("/sum", thttptransport.NewServer(
		endpoints.SumEndpoint,
		decodeHTTPSumRequest,
		thttptransport.EncodeResponseFuncAdapter[addendpoint.SumResponse](encodeHTTPGenericResponse),
		append(options, httptransport.ServerBefore(opentracing.HTTPToContext(otTracer, "Sum", logger)))...,
	))
	m.Handle("/concat", thttptransport.NewServer(
		endpoints.ConcatEndpoint,
		decodeHTTPConcatRequest,
		thttptransport.EncodeResponseFuncAdapter[addendpoint.ConcatResponse](encodeHTTPGenericResponse),
...
func NewHTTPClient(instance string, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, logger log.Logger) (addservice.Service, error) {
	...
	{
		sumEndpoint = thttptransport.NewClient[addendpoint.SumRequest, addendpoint.SumResponse](
			"POST",
			copyURL(u, "/sum"),
			thttptransport.EncodeRequestFuncAdapter[addendpoint.SumRequest](encodeHTTPGenericRequest),
	...
	{
		concatEndpoint = thttptransport.NewClient[addendpoint.ConcatRequest, addendpoint.ConcatResponse](
			"POST",
			copyURL(u, "/concat"),
			thttptransport.EncodeRequestFuncAdapter[addendpoint.ConcatRequest](encodeHTTPGenericRequest),
